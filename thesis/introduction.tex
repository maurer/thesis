...
We need program analysis techniques that can find security bugs in
binary programs. Binary program analysis is essential because code is
often only available in binary form, and even when source code is
available, some security properties are more appropriate at the binary
level.

   - Let's not over-explain many people don't' have source. Needs to be
   said, but don't dwell.
   - Second thing: sometimes binary code is the right place to perform
   analysis, e.g., buffer overflow more accurate if you know the size
   the compiler created not just what's stated in source. We may not
   want to say this since we don't have a buffer overflow detector,
   though. Is there a case for this for use-after-free?  What about
   types at the binary level?
...
In this model, an analysis author must ensure any information they
wish to consume has been calculated before their analysis, but after
any analyses that could benefit their dependency's accuracy. \todo{put
  in DAG here. You want to say explicitly that in many cases, e.g.,
  source code, a DAG is sufficient.  Recently Jackstab argued that
  there may be cycles in co-dependent analysis. They perform initial
  investigations, but are limited to a single cycle.  Further, they
  hardcoded the cycle.}
...
- We often need co-dependent analysis to detect bugs. This needs to be
said. Right now it's not really in the intro.  Then say in previous
work this is a known requirement (?).  Examples.  Then you need to
explicitly say why previous work is insufficient (either technically
or ``not good enough'').


\todo{This paragraph should start with: our main insight is that we
  need an analysis framework that allows for arbitrary
  cycles. Further, we want a framework that is extensible so that new
  analysis can be added easily.}
...
There are still some restrictions on how such code can operate (for
example, no preserved state across calls) but taking this approach
gives the flexibility to be an integration system.\todo{fix me up.}
