...
For this purpose I invented a novel form of negation, based on circumscription~\cite{circumscription}, which is suited to this particular application (\S~\ref{sec:inf-circ}).
I define the semantics of the resulting logic language in terms of a Kripke structure (\S~\ref{sec:formal}).
I demonstrate that this approach, as realized in \sysname, can be reasoned about by connecting to the semantics to a search strategy.
I show the search strategy will
\begin{itemize}
	\item Make progress (\S~\ref{sec:progress})
	\item Only output results sound with respect to the semantics (\S~\ref{sec:soundness})
	\item In a restricted set of situations, terminate (\S~\ref{sec:term})
\end{itemize}
I show that \sysname can implement different sensitivities of alias analysis, and show how this affects practical use-after-free classification (\S~\ref{sec:alias}).
I conclude that \sysname does solve the problem of co-dependence in program analysis and allow analyses to be specified in a modular fashion(\S~\ref{sec:conc}).
Finally, I discuss ways this approach could be extended to make it a more powerful and complete solution to the problem of analysis integration (\S~\ref{sec:future}).
