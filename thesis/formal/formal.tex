...
Perform simple replacement of all circumscripted matches to $P$ with matches against $P_c$ \emph{and} $P$ at the same indices.
...
\[
	\bot \leftarrow P_c(\vec{x}, \vec{y}) \& P(\vec{x}, \vec{z}) + \textrm{P-less-than}
\]
...
\[
	\infer[\textrm{consistent}]{\cons{\Pi}}{\Pi \not \models \bot & (\Pi \models P_c(\vec{x})) \imp (\Pi \models P(\vec{x}))}
\]
...
Since our lattices are not guaranteed to be bounded, we cannot provide a meaningful circumscription without at least one fact.
