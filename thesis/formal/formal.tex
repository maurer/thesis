...
%TODO add more detail in these steps
Using circumscriptive negation, we begin with the empty partial model.
Applying assume-false is not possible, because $\neg W(\Pi \cup (\neg P \leftarrow))$.
This modified program is not a world because it fails consistency since both $\neg P$ and $P$ are true at it (so $\neg C(\Pi \cup (\neg P \leftarrow))$).
In this case, we can apply assume-true.
The resulting program is $\{P \leftarrow \neg P, P \leftarrow\}$.
The minimal model for this program is $P$, and it decides every mentioned predicate.
As a result,
\[
P \leftarrow \neg P \models \boks \dia \boks P
\]
