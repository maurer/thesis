...
BiTR generates constraints for each register/program point combination based on perceived dataflow in the program.
...
These systems could all benefit from using each others' output to improve their results when possible, and from the ability to combine results of different methods for answering the same question, depending on how much precision is needed or what resources are available.
...
The version of backwards chaining employed by Prolog has explicit execution order, and so falls victim to being too inflexible to allow the scheduling of available resources.
...
Dependency cycles can be dealt with via fixpoint semantics when rules chain forwards, and a search procedure when chaining backwards.
...
While building this system, I will work to evaluate the technique's applicability to the domain. Specifically, I seek to answer:
\begin{itemize}
  ...
  \item What improvements to result quality or analysis efficiency can we gain from combining analyses?
...
The use of a logic language oriented approach to describing the relationship between analyses provides a unique opportunity to explore the implications of circumscription in incomplete logic languages.
...
One possible approach to this is a tactics script style language to allow the programmer to provide evaluation strategies to \sysname.
