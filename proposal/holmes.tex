...
Logic language as used in binary analysis has traditionally been relegated to a role of implementing a particular analysis, to be later integrated by custom code\cite{bddbddb,Alpuente2011,Brumley2006b,Lam2005a,Smaragdakis}.
...
Using strided interval notation\cite{vsa}\footnote{\
Strided intervals are a set notation described by a lower bound, an upper bound, and a stride.
The set includes the lower bound, and every element less than or equal to the upper bound which is a multiple of the stride above the lower bound.
It is written $\textrm{stride}[\textrm{lower}, \textrm{upper}]$.
}
\begin{gather*}
...
In the example problem, we dealt with upper bounds defined as strided intervals\footnote{\S~\ref{sec:cfgexample}}.
...
As mentioned earlier\footnote{\
        \S~\ref{sec:cfg} and \S~\ref{sec:valanal}
}, this procedure involves explicitly postulating that our current knowledge of control flow is complete, computing potential values, and determining whether our assumptions were violated, updating them if they were.
