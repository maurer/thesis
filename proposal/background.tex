...
\end{gather*}
This is potentially more interesting because it is a recursive build up of information, unlike the previous example which was considerable as a two-step procedure: find the siblings, then find siblings of parents.
...
Adding a large, but still finite bounded type like $\texttt{uint64}$ makes any numerical approximation of runtime from value count irrelevant, but still technicaly gives termination.
...
While not a hard and fast rule, these are usually properties that talk about many or all paths.
...
While there is no rule that says there must be types as we think of them in a binary program, they are usually present due to a source language or programmer volition.
